...
import { serviceImages, type ImageConfig } from "@/lib/images";
import {
  FiCheck,
  FiArrowRight,
  FiTrendingUp,
...
function getServiceImage(slug: string): ImageConfig {
...
export async function generateMetadata({
  ...
  if (!service) {
    return {
      title: "Service Not Found | VisdomWaves",
    };
  }

  return {
    title: `${service.name} | VisdomWaves Services`,
...
export default function ServiceDetailPage({ params }: ServicePageProps) {
  const service = getServiceBySlug(params.slug);
  const serviceImage = getServiceImage(params.slug);
...
function getFeatureIcon(serviceSlug: string, featureIndex: number) {
  // AI & Machine Learning Solutions
  if (serviceSlug === 'ai-machine-learning') {
    const aiIcons = [
      <FiCpu className="w-6 h-6 text-white" />, // Predictive Analytics
      <FiCode className="w-6 h-6 text-white" />, // NLP
      <FiImage className="w-6 h-6 text-white" />, // Computer Vision
      <FiBarChart2 className="w-6 h-6 text-white" />, // Recommendation Systems
      <FiGitBranch className="w-6 h-6 text-white" />, // AI Model Development
      <FiServer className="w-6 h-6 text-white" />, // Model Deployment & MLOps
    ];
  ...
  if (serviceSlug === 'digital-transformation') {
    const digitalIcons = [
      <FiMapPin className="w-6 h-6 text-white" />, // Digital Strategy & Roadmap
      <FiRefreshCw className="w-6 h-6 text-white" />, // Legacy System Modernization
      <FiSliders className="w-6 h-6 text-white" />, // Process Automation
      <FiCloud className="w-6 h-6 text-white" />, // Cloud Migration
      <FiMonitor className="w-6 h-6 text-white" />, // Digital Workplace
      <FiUsers className="w-6 h-6 text-white" />, // Change Management
    ];
  ...
  if (serviceSlug === 'web-development') {
    const webIcons = [
      <FiCode className="w-6 h-6 text-white" />, // Custom Web Applications
      <FiShoppingCart className="w-6 h-6 text-white" />, // E-Commerce Platforms
      <FiSmartphone className="w-6 h-6 text-white" />, // Progressive Web Apps
      <FiDatabase className="w-6 h-6 text-white" />, // API Development & Integration
      <FiShield className="w-6 h-6 text-white" />, // Security & Compliance
      <FiZap className="w-6 h-6 text-white" />, // Performance Optimization
    ];
