...
export function FloatingAssistant() {
  ...
  const processCommand = useCallback((message: string): { response: string; navigate?: string } => {
    const lowerMessage = message.toLowerCase().trim();

    const navigationMap: { [key: string]: { path: string; response: string } } = {
  ...
  const processAndSendMessage = useCallback((text: string) => {
    if (!text.trim()) return;
      ...
      {!isOpen && (
        <button
          ...
          className="flex items-center justify-center rounded-full bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-2xl transition-all hover:scale-110 hover:shadow-2xl
            ...
            right-4 sm:right-6
            bottom-20 sm:bottom-auto sm:top-[400px]"
          style={{ 
            border: 'none',
            cursor: 'pointer'
          }}
      ...
      {isOpen && (
        <div
          ...
          className={`flex flex-col bg-white shadow-2xl transition-all border border-gray-200
            ...
            bottom-0 sm:bottom-auto right-0 sm:right-6
            sm:top-20
              ...
              <div className="border-t border-gray-200 p-2 sm:p-3 bg-white rounded-b-xl sm:rounded-b-xl">
                ...
                <div className="flex gap-1.5 sm:gap-2">
                  ...
                  <button
                    ...
                    className={`rounded-full p-2 sm:p-2.5 text-white transition-all ${
                      isListening
